/** Katılımcının test öncesi gördüğü yönerge metni */
export const TEST_INSTRUCTION_TITLE = "FocusProlab Dikkat Testi Yönergesi";

/** Alıştırma ses kontrolü — public/audio-check.mp3 (mixkit-happy-home-801) */
export const AUDIO_CHECK_SOUND = "/audio-check.mp3";

export const TEST_INSTRUCTION_PARAGRAPHS = [
  "Bu testte ekranda farklı şekiller göreceksin.",
  "Senin görevin yalnızca mavi üçgeni her gördüğünde boşluk tuşuna en hızlı şekilde sadece bir kez basmaktır.",
  "Mavi üçgen dışında başka hiçbir şekli gördüğünde basma.",
  "Örneğin; mavi kare, yeşil üçgen, kırmızı daire, siyah artı veya başka şekiller gördüğünde basma.",
  "Yönergeden sonra 30 saniyelik çeldiricisiz bir deneme yapacaksın; bu deneme kaydedilmez.",
  "Unutma: Sadece mavi üçgeni her gördüğünde boşluk tuşuna en hızlı şekilde sadece bir kez basmalısın."
];

export const GUIDE_PRACTICE_BUTTON = "Deneme testi için tıklayınız";
